...
public class SiteAwareSchedulerwithJsonWithFailureNote implements IScheduler {
...
//            }
            ...
            for (String boltName : bolts.keySet()) {
                ...
                if(executors!=null)
                {
                    ...
                    if(executors.size()!=Integer.parseInt(thread_count_in_conf))
                    {//checking conf_thread --> (topo_exec passed/Rebalance_exec_passed)
                        ...
                        System.out.println("\n\n\t\t\t\t*****Rebalance/New submission--Toponame********"+topoName+"****BoltName****"+boltName);
                    }
            ...
            for (String spoutName : spouts.keySet()) {
                ...
                if(executors!=null && needsSchedulingFlagBolt==1)//Flag checking: if condition for all bolts are fine the inly go for spout
                {
                    ...
                    System.out.println("\n\n\t\t\t\t*****Rebalance/New submission--Toponame********"+topoName+"****SpoutName****"+spoutName);
                }
